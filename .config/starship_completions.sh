_starship() {
    ...
    do
        case "${cmd},${i}" in
            ...
            starship__help,config)
                ...
                ;;
            starship__help,config-schema)
                cmd="starship__help__config__schema"
                ;;
    ...
    case "${cmd}" in
        starship)
            opts="-h -V --help --version bug-report completions config explain init module preset print-config prompt session time timings toggle config-schema help"
        ...
        starship__explain)
            ...
            ;;
        starship__help)
            opts="bug-report completions config explain init module preset print-config prompt session time timings toggle config-schema help"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
        ...
        starship__help__config)
            ...
            return 0
            ;;
        starship__help__config__schema)
            opts=""
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 3 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
