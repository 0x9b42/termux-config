...
#!/bin/bash
MONITOR_DIR="/data/local/tmp"
BACKUP_DIR="/sdcard/dex_dump"
LOG_FILE="$BACKUP_DIR/dex_capture.log"
EVENTS="create,close_write,delete"
FORMAT="%T | %w%f | %e"
TIME_FMT="%Y-%m-%d_%H:%M:%S"
mkdir -p "$BACKUP_DIR"
echo "==== Monitoring dimulai: $(date) ====" >"$LOG_FILE"
inotifywait -m -r -e "$EVENTS" --timefmt "$TIME_FMT" --format "$FORMAT" "$MONITOR_DIR"|while read -r LINE;do
...
case "$EVENT_TYPE" in
"CREATE"|"CLOSE_WRITE")if
! lsof "$FILE_PATH" >/dev/null
then
...
;;
"DELETE")echo "[!] DEX deleted: $FILE_PATH"|tee -a "$LOG_FILE"
esac
fi
done
